...
public class CourseManagementActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Listen for real-time updates from the courses collection.
     ...
     * against both the course name and its acronym.
     */
    private void applyFilter() {
        ...
        for (DocumentSnapshot doc : all) {
            ...
            String college = resolveCollegeName(doc).toLowerCase();
            if (q.isEmpty() || name.contains(q) || acronym.contains(q) || campus.contains(q) || college.contains(q)) filtered.add(doc);
        }
    ...
    private class CrudAdapter extends RecyclerView.Adapter<CrudAdapter.VH> {
        ...
        public void onBindViewHolder(VH h, int pos) {
            ...
            String college = resolveCollegeName(doc);
            String desc = orDash(doc.getString("description"));
            if (!"—".equals(campus) || !"—".equals(college)) {
                desc = desc + "\nCampus: " + campus + "\nCollege: " + college;
            }
            ...
            h.btnEdit.setOnClickListener(v -> {
                Intent i = new Intent(CourseManagementActivity.this, CourseFormActivity.class);
                i.putExtra(CourseFormActivity.EXTRA_DOC_ID, doc.getId());
                startActivity(i);
            });
    ...
    private String resolveCollegeName(DocumentSnapshot doc) {
        String name = doc.getString("collegeName");
        if (name == null || name.isEmpty()) name = doc.getString("collegeLabel");
        if (name == null || name.isEmpty()) name = doc.getString("college");
        return orDash(name);
    }
}